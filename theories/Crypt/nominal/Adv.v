...
Lemma Adv_perf_l {P P' Q A : nom_package} {E}
  {V1 : ValidPackage (loc P) Game_import E P}
  {V2 : ValidPackage (loc P') Game_import E P'}
  {V3 : ValidPackage (loc A) E A_export A} :
...
Lemma Adv_perf_r {P P' Q A : nom_package} {E}
  {V1 : ValidPackage (loc P) Game_import E P}
  {V2 : ValidPackage (loc P') Game_import E P'}
  {V3 : ValidPackage (loc A) E A_export A} :
...
Definition perfect I G G' :=
  ∀ (A : nom_package)
    (VA : ValidPackage (loc A) I A_export A), Adv G G' A = 0.

Lemma perfect_refl {I G} : perfect I G G.
Proof. intros A VA. rewrite Adv_same //. Qed.

Lemma perfect_sym {I G G'} : perfect I G' G → perfect I G G'.
Proof. intros H A VA. rewrite Adv_sym H //. Qed.
...
Lemma Adv_perfect_l {E} {P P' Q A : nom_package}
  {V : ValidPackage (loc A) E A_export A} :
...
Lemma Adv_perfect_r {E} {P P' Q A : nom_package}
  {V : ValidPackage (loc A) E A_export A} :
...
Lemma prove_perfect {E} {G G' : nom_package} :
  ∀ (V  : ValidPackage (loc G) Game_import E G )
    (V' : ValidPackage (loc G') Game_import E G'),
    G ≈₀ G' → perfect E G G'.
Proof. intros V V' H A VA. apply (Adv_perf H _ VA). Qed.

Lemma Adv_par_link_r (P₀ P₁ P₁' G G' A : nom_package)
  {I₀ I₁ E₀ E₁}
  {V1 : ValidPackage (loc P₀) I₀ E₀ P₀}
  {V2 : ValidPackage (loc P₁) I₁ E₁ P₁}
  {V3 : ValidPackage (loc P₁') I₁ E₁ P₁'} :
  fseparate I₀ (val P₁) →
  fseparate I₀ (val P₁') →
  Adv ((P₀ || P₁) ∘ G) ((P₀ || P₁') ∘ G') A
...
Lemma Adv_par_r (G₀ G₁ G₁' A : nom_package) {E₀ E₁}
  {V1 : ValidPackage (loc G₀) Game_import E₀ G₀}
  {V2 : ValidPackage (loc G₁) Game_import E₁ G₁}
  {V3 : ValidPackage (loc G₁') Game_import E₁ G₁'} :
...
Proof.
  ...
  rewrite (@sep_par_factor_l _ _ _ _ _ G₁').
  2: fmap_solve.
  rewrite sep_par_empty_l.
  reflexivity.
Qed.
...
Lemma Adv_par_l (G₀ G₀' G₁ A : nom_package) {E₀ E₁}
  {V1 : ValidPackage (loc G₀) Game_import E₀ G₀}
  {V2 : ValidPackage (loc G₀') Game_import E₀ G₀'}
  {V3 : ValidPackage (loc G₁) Game_import E₁ G₁} :
...
Proof.
  ...
  rewrite (@sep_par_factor_r _ _ _ _ G₀').
  2: fmap_solve.
  rewrite sep_par_empty_r.
  reflexivity.
Qed.
...
Lemma AdvOf_perfect {E} {G G'} {A : nom_package}
  {V : ValidPackage (loc A) E A_export A} :
