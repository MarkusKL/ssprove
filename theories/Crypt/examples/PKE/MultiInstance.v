...
Definition pks_loc : Location := (51%N, chMap nat P.(Pub)).
...
Definition counts_loc : Location := (52%N, chMap nat nat).
...
Proof.
  ...
  - destruct arg as [j m]. simpl.
    ...
    + rewrite E /=. simpl in *. ssprove_code_simpl.
      apply r_get_vs_get_remember => [pks].
      ssprove_code_simpl_more.
      ssprove_code_simpl.
      ssprove_code_simpl_more.
...
Theorem Adv_MI_CPA_SI q A `{IsAdversary (I_MCPA P n) A} :
...
Section MultiElgamal.

Context (n q : nat) `{Positive n}.
Context (G : CyclicGroup).
Let E := elgamal G.

Notation STM := (HMCPA E n q ∘ (ID (I_CPA E) || RAND (unif n)))%sep.
Notation OTM := (SLIDE E q ∘ (ID (I_CPA E) || RAND (unif q)))%sep.

Lemma Adv_MI_CPA_OT {A} `{IsAdversary (I_MCPA E n) A} :
  AdvOf (λ b, MCOUNT E n q ∘ MCPA E n b)%sep A
    = (AdvOf (LDDH G) (A ∘ STM ∘ OTM ∘ RED G) *+ q *+ n)%R.
Proof.
  etransitivity.
  1: apply Adv_MI_CPA_SI; ssprove_valid.
  rewrite MT_CPA_elgamal.
